...
// Definisi format output otomatis
const fileTypes = {
  jpg: ["png", "webp"],
  png: ["jpg", "webp"],
  gif: ["png", "webp"],
...
// Data artikel untuk bagian blog/artikel
const articles = [
  {
  ...
  },
  {
    ...
    content: `
      ...
      <ol class="list-decimal pl-5 mb-4">
        ...
        <li>Setelah video selesai dikonversi, kamu siap meng-upload video ke YouTube.</li>
      </ol>
    
      <h2 class="text-2xl font-semibold mb-4">Tips Mengoptimalkan Video untuk Upload yang Lebih Cepat dan Kualitas Terbaik</h2>
      <p class="mb-4">Selain memilih format video yang tepat, ada beberapa tips tambahan untuk memastikan video Anda dioptimalkan dengan baik sebelum diunggah ke platform:</p>
  ...
  },
  {
    ...
    content: `
      ...
      <ul class="list-disc pl-5 mb-4">
        <li><strong>Gunakan bitrate yang cukup tinggi untuk menjaga kualitas suara tetap jelas dan tajam, tetapi jangan terlalu tinggi hingga file menjadi terlalu besar (128 kbps atau 192 kbps adalah pilihan yang baik).</strong></li>
        <li><strong>Pastikan audio bebas dari noise dan gangguan. Gunakan perangkat perekaman yang berkualitas dan lakukan editing untuk menghilangkan suara latar yang tidak diinginkan.</strong></li>
        <li><strong>Perhatikan keseimbangan antara suara pembicara, musik latar, dan efek suara. Pastikan suara pembicara tetap jelas dan tidak terhalang oleh suara latar atau musik.</strong></li>
        <li><strong>Gunakan alat editing audio untuk memperbaiki kualitas suara, seperti normalisasi volume, pengurangan noise, dan equalizer.</strong></li>
        <li><strong>Periksa panjang file audio agar tetap nyaman didengarkan. Sebagian besar pendengar lebih suka episode podcast yang tidak terlalu panjang dan lebih mudah diikuti.</strong></li>
      </ul>
...
const ArticleList = () => {
  return (
    <motion.div
      ...
      className="bg-white p-6 rounded-lg shadow-xl w-full max-w-4xl mx-auto"
    >
      <h2 className="text-3xl font-bold text-gray-800 mb-6 text-center">Artikel Terbaru</h2>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {articles.map((article) => (
          <Link to={`/articles/${article.slug}`} key={article.slug}>
            <div className="bg-gray-50 p-4 rounded-lg shadow hover:shadow-md transition-shadow duration-200 cursor-pointer">
              <h3 className="text-xl font-semibold text-indigo-600 mb-2">{article.title}</h3>
              <p className="text-sm text-gray-500 mb-2">Tanggal: {article.date}</p>
              <p className="text-gray-700 text-sm line-clamp-3" dangerouslySetInnerHTML={{ __html: article.content }}></p>
              <span className="text-indigo-500 text-sm mt-2 block">Baca Selengkapnya &rarr;</span>
            </div>
          </Link>
        ))}
      </div>
...
const ArticleDetail = () => {
  ...
  if (!article) {
    return (
      <motion.div
        ...
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
        className="bg-white p-6 rounded-lg shadow-xl w-full max-w-4xl mx-auto text-center"
      >
  ...
  return (
    ...
    >
      ...
      <p className="text-gray-500 text-sm mb-6">Tanggal Publikasi: {article.date}</p>
      <div className="prose max-w-none text-gray-700 leading-relaxed" dangerouslySetInnerHTML={{ __html: article.content }} />
      <button onClick={() => navigate("/articles")} className="mt-8 btn-secondary">
...
// Global styles for modals (moved from UploadBox for global availability)
const globalStyles = `
  .modal {
    position: fixed;
    z-index: 1000;
  ...
  .message-modal-content {
    ...
    box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1), 0 4px 6px -2px rgba(0, 0, 0, 0.05);
    width: 90%;
    max-width: 400px; /* Slightly smaller for message modals */
    position: relative;
    text-align: center;
  }
  .modal-close-button {
  ...
  /* Custom button styles from Kaspocket (for message modal OK button) */
  .btn-primary {
      display: inline-flex;
      justify-content: center;
      align-items: center;
      ...
      transition-duration: 150ms; /* transition-colors duration-200 */
      cursor: pointer;
  }
  .btn-primary:hover {
  ...
  .btn-primary:focus {
      ...
      outline-offset: 2px;
      box-shadow: ring-2 ring-offset-2 ring-indigo-500; /* focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 */
  }
`;
...
function App() {
  ...
  return (
    <HelmetProvider>
      ...
      {/* Main layout container with flex for side ads */}
      <div className="flex flex-col lg:flex-row justify-center items-stretch min-h-screen bg-gray-100 font-inter">
        {/* Helmet for SEO */}
        <Helmet>
          <title>FajrConvert - Konversi File Online Gratis dan Cepat</title>
          <meta
            name="description"
            content="Konversi berbagai format file (gambar, audio, video, dokumen, arsip) secara online dan gratis dengan FajrConvert. Mudah, cepat, dan aman."
          />
        ...
        <div className="flex-grow flex flex-col items-center p-4 sm:p-6 lg:p-8">
          {/* Header Section */}
          <header className="w-full max-w-4xl mx-auto flex flex-col sm:flex-row justify-between items-center py-4 px-2 sm:px-0">
            {/* Logo and title */}
            <Link to="/" className="flex items-center space-x-2">
              <img src={_logoconverfajr} alt="FajrConvert Logo" className="h-12 w-auto rounded-md" />
              <h1 className="text-3xl font-bold text-gray-800">FajrConvert</h1>
            </Link>
            {/* Navigation (Links) */}
            <nav className="mt-4 sm:mt-0">
              <ul className="flex space-x-4">
                <li>
                  <Link to="/" className="text-gray-600 hover:text-indigo-600 font-medium transition-colors duration-200">Beranda</Link>
                </li>
                <li>
                  <Link to="/articles" className="text-gray-600 hover:text-indigo-600 font-medium transition-colors duration-200">Artikel</Link>
                </li>
                <li>
                  <Link to="/privacy-policy" className="text-gray-600 hover:text-indigo-600 font-medium transition-colors duration-200">Privasi</Link>
                </li>
              </ul>
            </nav>
          </header>
